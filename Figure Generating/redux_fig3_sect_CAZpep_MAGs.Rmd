...
MAGs_p$`MAG CAZyme` = size_CAZ
MAGs_p$`MAG peptidase` = size_pep

MAGs.melt = MAGs_p[c(id.col, c("MAG CAZyme", "MAG peptidase"))] %>% 
  reshape2::melt(id = id.col) %>%
  mutate(gene = variable,
         percent.CAZ.pep = value)

gs.particle.stats = MAGs.melt %>%
  group_by(is_deep, gene) %>%
  wilcox_test(complete_genome_size~percent.CAZ.pep) %>%
  adjust_pvalue(method = "BY") %>%
  filter(p.adj < 0.05) %>%
  add_significance()

gs.particle.stats$xmin = c(1,1,1,1,2,1,1,1,1,2)
gs.particle.stats$xmax = c(2,3,2,3,3,2,3,2,3,3)
gs.particle.stats$y.position = log10(c(10,12,9,11,14.5,10,12,11,13.5,17.5) )

genome.size_particle = MAGs.melt %>% # 
  ggplot(aes(x=percent.CAZ.pep, y=complete_genome_size)) +
  geom_boxplot(outlier.shape = NA, varwidth = TRUE) +
  scale_color_viridis(option = "H",
                      breaks = trans_scale, labels = trans_scale,
                      name = "\nTransposase abundance (%)", trans = "log") +
  facet_grid(is_deep~gene, scale = "free_x") +
  geom_jitter(aes(color = trans_fudge), alpha = 0.4, size = 0.7, width = 0.15) +
  xlab("secretory CAZyme/peptidase (%)") +
...
class.median = MAGs_p %>%
  ...
  group_by(Class) %>%
  mutate(n = n()) %>%
  filter(n >= 10 & !Class == "Others Or Unknown" & !Class == "other") %>%
  summarise(median_CAZ = mean(percent_sect_CAZ),
...
```{r}
MAGs_p2 = MAGs_p %>%
  filter(Class %in% class.median$Class) %>%
  mutate(Class = factor(Class, levels = class.median$Class),
          CAZyme = percent_sect_CAZ,
          peptidase = percent_sect_pep)

id.v2 = c(id.col, "Class")
tax.melt = MAGs_p2[c(id.v2, c("CAZyme", "peptidase"))] %>% 
...
tax.melt %>%
  ...
  ggplot(aes(x=percent.CAZ.pep, y=gene)) +
  geom_boxplot(outlier.alpha = 0.3, alpha = 0.5) +
  facet_wrap(~Class, ncol = 4) +
  labs(x = "secretory CAZyme/peptidase ORFs (%)") +
  theme_classic() +
  theme(axis.title.y = element_blank()) +
  # geom_jitter(aes(color = log(trans_fudge))) +
  geom_text(aes(label=n, x =65, y = 1.5),size=3)

ggsave("out/S5-class-secretory-CAZyme-peptidase.pdf",
       height = 4, width = 7)

MAGs_p2 %>%
  ggplot(aes(y = fct_rev(Class), x = trans_fudge)) +
  geom_boxplot() +
  scale_x_log10() +
  labs(x = "transposase ORF in MAGs (%)",
       y = "class") +
  theme_classic() +
  stat_summary(fun.data = boxplot.give.nr, 
