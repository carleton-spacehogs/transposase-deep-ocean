...
MAGs.melt = MAGs_p[c(id.col, c("CAZ_fudge", "percent_sect_pep"))] %>% 
  ...
  mutate(gene = ifelse(variable == "CAZ_fudge", 
                       "CAZymes in MAGs", "peptidases in MAGs"))
...
class.median = MAGs_p %>%
  ...
  group_by(Class) %>%
  summarise(count =n(),
    median_CAZ = median(percent_sect_CAZ),
    median_pep = median(percent_sect_pep)) %>%
...
# class.count = str_replace(class.count, "OM190 \\(", "OM190 \\(under Planctomycetes, ")
...
## particle association (cut) vs genome size
```{r}
