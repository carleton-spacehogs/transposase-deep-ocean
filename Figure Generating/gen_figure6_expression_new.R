...
gen_plot <- function(gene_name, y_text, log_scale, percent_scale, upscale, 
  ...
  ggplot(sum_bar, aes(y=fct_rev(depth), x=median_abun, fill=fct_rev(type))) + 
    ...
    xlab(y_text) +
    annotate(geom="text", x=x_max*txt_top, y=3.25, label="DNA") + 
    annotate(geom="text", x=x_max*txt_bot, y=2.8, label="RNA") + 
    theme_classic() +
...
y2 <- "% reads mapped to defense mech. ORFs"
...
library(png)
img <- readPNG("./F6_exp_ratio_table.png")
exp_table_img <- ggplot() + background_image(img) + 
  theme(plot.margin = margin(t=0.5, l=0.2, r=0.01, b=0.8, unit = "cm"))

ggarrange(
  ggarrange(pCassette, exp_table_img, 
            ncol = 1, heights = c(0.6, 0.3), labels = c("A", "B")),
...
ggsave("F6_expression_and_cassette_pnps_bars2.png", plot = last_plot())
