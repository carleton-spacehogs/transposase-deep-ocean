...
pretty.exp = c("Extracellular structures", "Mobilome prophages transposons",
  "Defense mechanisms", "Replication recombination and repair",
  "Signal transduction mechanisms", "Transcription",
  "Cell motility", "Transposase")
...
exp.melt %>%
  ...
  facet_wrap(~pretty.gene, scales = "free_x", ncol = num.col) + 
...
```{r}
D.RNA = c("Extracellular_struct", "Mobilome_prophages_transposons",
            "Defense_mechanisms", "Replication_recombination_and_repair",
            "Signal_transduction_mechanisms", "Transcription",
            "Cell_motility", "transposase")
D.RNA.col = c("Layer_RNA", paste0("DNA_", D.RNA), paste0("RNA_", D.RNA))
...
DNA.RNA.melt$gene = gsub("transposase", "Transposase", DNA.RNA.melt$gene)
DNA.RNA.melt$gene = gsub("struct", "Structures", DNA.RNA.melt$gene)

DNA.RNA.melt %>%
  mutate(Type = ifelse(Type == "RNA", "transcript (RNA)", "gene (DNA)")) %>%
  ggplot(aes(y=fct_rev(Layer_RNA), x=value*100)) + 
  geom_boxplot(aes(fill = Type), outlier.alpha = 0.3) +
  facet_wrap(~gene, scale = "free_x") + 
  scale_fill_manual(values = c(`transcript (RNA)` = "orange", `gene (DNA)` = "gray")) +
  scale_x_log10() +
  theme_classic() +
  xlab("gene / transcript abundance (%)") +
  theme(axis.title.y = element_blank(),
...
dna.rna.out.f = "/S7-integron-COGs-DNA-RNA.pdf"
ggsave(paste0("out", dna.rna.out.f), height = 5, width = 7)
```
