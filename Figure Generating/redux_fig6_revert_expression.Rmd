...
```{r}
gene_col =c("secretory_CAZyme", "secretory_peptidase", # "biofilm",
            "Defense_mechanisms", "transposase")

pretty.exp = c("secretory CAZyme", "secretory peptidase", # "biofilm",
               "defense mechanisms", "transposase")

out.f = "/Figure6-expression-ratios-with-biofilm.pdf"
```
...
# with biofilm
# exp.stats$y.position = log10(c( 2.5, 3, 1.1, 1.9, 3.9, 4.4, 1.2, 1.4))

# without biofilm
exp.stats$y.position = log10(c( 2.7, 3.5, 1.1, 1.2, 1.4))
...
ggsave(paste0("out", out.f), height = 3, width = 4.5) # 6.5 with biofilm
ggsave(paste0(googleDrive.loc, out.f), height = 3, width = 4.5) # 6.5
```
...
tmp = DNA.RNA.melt %>%
  ...
  scale_x_log10() +
  xlab("transcript and gene abundance (%)")
...
## Biofilm special!
```{r}
biofilm_df = DNA_RNA[c("Layer_RNA", "biofilm_exp", "DNA_biofilm", "RNA_biofilm")]
biofilm_df$depth = fct_rev(biofilm_df$Layer_RNA)

biofilm.stats = biofilm_df %>%
  wilcox_test(biofilm_exp~depth) %>%
  adjust_pvalue(method = "BH") %>%
  filter(p.adj < 0.05) %>%
  add_significance() %>% 
  add_xy_position() %>%
  mutate(y.position = log10(c(3.9, 4.6)))

tmp = biofilm_df %>%
  ggplot(aes(y = biofilm_exp, x = depth)) +
  scale_y_continuous(trans = "log10",
    labels = function(x) ifelse(x == 0, "0", x)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.2, width = 0.2, size = 1) +
  stat_pvalue_manual(biofilm.stats, tip.length = 0.01,
    label = "p.adj.signif", hide.ns = TRUE ) +
  coord_flip() +
  ylab("RNA/DNA ratio (transcript / gene abundance)") +
  ggtitle("biofilm")

biofilm.exp = format_boxplots(tmp) +
  theme(plot.title = element_text(hjust = 0.5))

biofilm.melt = biofilm_df[c("depth", "DNA_biofilm", "RNA_biofilm")] %>% reshape2::melt(id = "depth")

tmp = biofilm.melt %>%
  mutate(Type = ifelse(variable == "RNA_biofilm", "transcript", "gene")) %>%
  ggplot(aes(y=depth, x=value*100)) + 
  geom_boxplot(aes(fill = Type), outlier.alpha = 0.3) +
  scale_fill_manual(values = c(`transcript` = "orange", `gene` = "gray")) +
  scale_x_log10() +
  xlab("transcript and gene abundance (%)") +
...
biofilm.dna_rna = format_boxplots(tmp) +
  theme(plot.title = element_text(hjust = 0.5))

ggarrange(biofilm.exp, biofilm.dna_rna, labels = c("A", "B"),
          ncol = 2, widths = c(5,6.5))

biofilm.out.f = "/biofilm-expression-DNA-RNA.pdf"
ggsave(paste0("out", biofilm.out.f), height = 3, width = 8)
```
