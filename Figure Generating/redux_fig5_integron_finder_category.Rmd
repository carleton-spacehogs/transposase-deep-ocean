...
graphing %>% 
  ...
  annotate(geom="text", x=18.6, y=0.106, size = 3.3,
...
COG_enrichment.stat = apply(summary, 1, function(row){
  ...
  p.val = prop.test(x = c(int_count, integron_total), n = c(nor_count, norm_total))$p.val
  return(p.val)
})
...
COG_enrichment.stat = t(apply(summary, 1, function(r){
  ...
  group2 = int_count + nor_count
  # test for over-representation (enrichment)
  p.val1 = phyper(int_count-1, nor_count, norm_total-nor_count, integron_total, lower.tail= FALSE)
...
colnames(enrich) = c("COG_function","p.val")
enrich = enrich %>%
  adjust_pvalue(method = "BH") %>%
  add_significance() %>%
  filter(p.val.adj < 0.05)

summary.stats = merge(summary, enrich, by="COG_function", all.x=TRUE)
