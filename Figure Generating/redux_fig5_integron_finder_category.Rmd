...
#   p.val = prop.test(x = c(int_count, integron_total), 
#                     n = c(nor_count, norm_total))$p.val
#   return(c(r['COG_function'], p.val))
# }))
# 
# colnames(COG_enrichment.stat) = c("COG","p.val")
# COG_enrichment.stat = COG_enrichment.stat %>%
#   as.data.frame() %>%
#   adjust_pvalue(method = "BY") %>%
#   add_significance()
# 
# rownames(COG_enrichment.stat) = COG_enrichment.stat$COG
# COG_sig = COG_enrichment.stat[integron_order, ]
