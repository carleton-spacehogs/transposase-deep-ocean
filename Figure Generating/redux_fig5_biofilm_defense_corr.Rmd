...
mala_cov = mala_cov[ mala_cov$size_fraction == "planktonic", ]
...
COG.corr$Layer_DNA <- factor(COG.corr$Layer_DNA, levels = d_level)
```
...
```{r}
gen_plot = function(df, fea1){
  g = ggplot(df, aes_string(x = fea1, y = "COG_sum_coverage")) +
...
CAZ.BAT = gen_plot(BAT.COG.corr, "log(percent_dna_sect_CAZ)")
pep.BAT = gen_plot(BAT.COG.corr, "log(percent_dna_sect_pep)")

ggarrange(CAZ.BAT, pep.BAT)

CAZ.tara = gen_plot(tara.COG.corr, "log(percent_dna_sect_CAZ)")
pep.tara = gen_plot(tara.COG.corr, "log(percent_dna_sect_pep)")

ggarrange(CAZ.tara, pep.tara)
```
...
get_COG_corr = function(COG.melt, feature) {
  ...
  tmp = tmp %>%
    ...
    mutate(COG_prop = COG_abun, #/COG_sum_coverage,
           is_deep = ifelse(Layer_DNA == "BAT", "BAT", "tara"))
  
  COG.stats = tmp %>%
  # group_by(COG) %>%
  group_by(COG, is_deep) %>%
  summarize(p.val = cor.test(fea, COG_prop, method = "spearman")$p.val,
            estimate = cor.test(fea, COG_prop, method = "sp")$estimate,
            count = n()) %>%
