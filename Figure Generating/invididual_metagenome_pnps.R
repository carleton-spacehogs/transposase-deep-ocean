...
p1 <- pn_ps_metagenome %>%
  filter(log_pnps < 1.4) %>%
  filter(log_pnps > -3) %>%
  filter(gene_type %in% c("n", "transposase")) %>%
  ggplot(aes(x=log_pnps, fill = fct_rev(gene_type))) +
...
ggarrange(p1, p2, ncol = 2, nrow = 1, widths = c(0.4,0.5),labels = c("A", "B"))
